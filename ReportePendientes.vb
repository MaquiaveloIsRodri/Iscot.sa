	...
	do while not xRs.EOF
		...
		if not cc = xRs("CENTROCOSTOS").Value then
			do while calculoDifMes > countMonth
				...
				query2 = "EXEC SP_OrdenServicio_PendientesHastaTotalxPeriodo @FechaHasta = '" & fechaHastaStr & "' , '202310' = '" & Periodo & "', @CC = '" & xRs("CENTROCOSTOS").Value & "' "
				set xRs2	= ConsultarSQL(query2, Self.Workspace)
				do while not xRs2.EOF
					HojaExcel.ActiveSheet.Cells(row, column).Value 		= xRs2("TOTAL").Value
					column = column + 1
					xRs2.MoveNext
				loop
				HojaExcel.ActiveSheet.Cells(row, 1).Value 		= xRs("CENTROCOSTOS").Value
				FPerido = DateAdd("m", -countMonth, fechaHasta)
				row = row + 1
			loop
			FPerido = fechaHasta
			cc = xRs("CENTROCOSTOS").Value
