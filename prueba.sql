...
/****** Object:  StoredProcedure [dbo].[SP_Ventas_ComposicionMensual]    Script Date: 16/04/2024 13:34:53 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_Ventas_ComposicionMensual]
...
BEGIN
	...
	FROM (
		...
		,(SELECT TOP 1 ep.id AS CodEP FROM UD_ESTADOFACTURACION AS oef WITH(NOLOCK) 
        inner JOIN ITEMTIPOCLASIFICADOR AS ep WITH(NOLOCK) ON oef.ESTADOPAGO_ID = ep.ID
        WHERE oef.BO_PLACE_ID = uos.ESTADOFACTURA_ID 
        ORDER BY oef.FECHA DESC) EstadoVenta
		...
		AND fa.ESTADO = 'C'
		AND fa.EXTERNAL_ID = ''
		AND LEFT(uos.FECHASERVICIO, 6) = @Periodo
		AND uios.FLAGITEM_ID = '{D6B3920D-C3FA-4B1F-921E-D653E3B9E170}'		-- Facturado
		GROUP BY cli.ID, per.NOMBRE,ESTADOFACTURA_ID
		...
		AND RIGHT(uios.PERIODOPERDIDA, 4) + LEFT(uios.PERIODOPERDIDA, 2) = @Periodo
		GROUP BY cli.ID, per.NOMBRE,ESTADOFACTURA_ID
