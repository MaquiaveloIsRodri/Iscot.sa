' Neuronal Intercambio Epistolar - Abierto
Sub Main
stop
	if Not aObject.IsNew then
        If aObject.Workspace.InState(3) = true Then 
            if aObject.NOTA <> "EN INTERCAMBIO" then
                If aObject.boextension.CONDICION Is Nothing Then
                    ...
                    Exit Sub
                End If
                If UCase(NombreUsuario()) <> "TRANSACTOR1" Then
                    If aObject.boextension.CONDICION.id  = "{A4CA0C19-0D12-4ED0-85B1-65AB82C47278}" OR  aObject.boextension.CONDICION.id = "{D1401BBF-2F9B-4BD0-AC3C-CA41FC995690}" Then
                        If MsgBox("Genera automaticamente el intercambio ? " , 36, "Pregunta") = 6 Then 
                            ...
                            set oMotivo  		    = ExisteBo(aObject, "ITEMTIPOCLASIFICADOR", "id", "{2E1F4B42-3133-44AB-B423-F1A965616444}", oMotivos.Valores , True, False, "=")

                            Set xIntercambio.motivo = oMotivo
