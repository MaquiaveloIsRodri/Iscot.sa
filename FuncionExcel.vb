...
sub main
	...
	HojaExcel.ActiveSheet.Cells(5, 3).Value 		= NombreMes(cdate(date)) & "-" & Year(CDate(date))
    HojaExcel.ActiveSheet.Cells(5, 4).Value 		= NombreMes(DateAdd("m", -1, Date)) & "-" & Year(DateAdd("m", -1, Date))
    HojaExcel.ActiveSheet.Cells(5, 5).Value 		= NombreMes(DateAdd("m", -2, Date)) & "-" & Year(DateAdd("m", -2, Date))
	...
	I = 6
	do while not xRs.EOF
        total = total + CDbl(xRs("TOTAL").Value)
		if cc <> xRs("CENTROCOSTOS").Value then
			if Cc <> "" Then
		...
		Select Case xRs("PERIODO").Value
			case year(date) & Right("0" & CStr(Month(Date)), 2)
				MontoActual = MontoActual + CDbl(xRs("TOTAL").Value)
                TotalActual = TotalActual + CDbl(xRs("TOTAL").Value)
			case year(date) &  Right("0" & CStr(Month(DateAdd("m", -2, Date))), 2)
				Monto2da 	= Monto2da + CDbl(xRs("TOTAL").Value)
                Total2Da    = Total2Da + CDbl(xRs("TOTAL").Value)
			case year(date) &  Right("0" & CStr(Month(DateAdd("m", -2, Date))), 2)
				Monto3ra = Monto3ra + CDbl(xRs("TOTAL").Value)
                Total3ra = Total3ra + CDbl(xRs("TOTAL").Value)
			Case Else
				resto      = resto + CDbl(xRs("TOTAL").Value)
                TotalResto = TotalResto + CDbl(xRs("TOTAL").Value)
		End Select
		xRs.MoveNext
	loop
	HojaExcel.ActiveSheet.Cells(I, 3).Value 		= MontoActual
	...
	HojaExcel.ActiveSheet.Cells(I + 1, 1).Value 	    = "Total"
	HojaExcel.ActiveSheet.Cells(I + 1, 2).Value 		= total
    HojaExcel.ActiveSheet.Cells(I + 1, 3).Value 		= TotalActual
    HojaExcel.ActiveSheet.Cells(I + 1, 4).Value 		= Total2Da
    HojaExcel.ActiveSheet.Cells(I + 1, 5).Value 		= Total3ra
    HojaExcel.ActiveSheet.Cells(I + 1, 6).Value 		= TotalResto
