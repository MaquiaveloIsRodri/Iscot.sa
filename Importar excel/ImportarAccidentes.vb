...
sub main
    ...
    for each xItem in aObject.ItemsTransaccion
		If UCASE(classname(xItem.Referencia)) = "PRODUCTO" Then
		   ...
		   End If
        End If
        If UCASE(classname(xItem.Referencia)) = "SERVICIO" Then
		   set xImpObras = Nothing
		   Set xImpObras = GetPosicionImpuesto(xItem.referencia, "Loc. Gan. Loc. Obras y Servicios")
		   If Not xImpObras Is Nothing Then
	          posObras = xImpObras.PosicionImpuesto.name
		      if posObras = "No Gravado"  then
	             AddFailedConstraint "Servicio " & xItem.referencia.descripcion & " tiene Loc. Gan. Loc. Obras y Servicios como No Gravado" & posObras, 0
              end if
		   Else
		      AddFailedConstraint "Servicio " & xItem.referencia.descripcion & " no tiene cargada posicion de impuesto Loc. Gan. Loc. Obras y Servicios" & posObras, 0
		   End If
        End If
	next
end sub