...
sub main
	...
	HojaExcel.Sheets.Add
	HojaExcel.Sheets("Hoja" & CStr(numero_hoja)).Name = "ZZ RESUMEN"
	HojaExcel.Sheets("ZZ RESUMEN").Select
	...
	calculoDifMes = DateDiff("m",fechaHasta, date())
	FPerido = Date()
	countMonth = 0
	column = 2
	row = 6
	...
	do while not xRs.EOF
		call ProgressControlAvance(Self.Workspace, "CC: " & xRs("CENTROCOSTOS").Value & vbNewLine & "OS: " & xRs("OS").Value)
		if not cc = xRs("CENTROCOSTOS").Value then
			do while calculoDifMes > countMonth
				countMonth = countMonth + 1
				Periodo = Year(FPerido) & Right("00" & Month(FPerido), 2) 

				query2 = "EXEC SP_OrdenServicio_PendientesHastaTotalxPeriodo @FechaHasta = '" & fechaHastaStr & "' , @Periodo = '" & Periodo & "', @CC = '" & xRs("CENTROCOSTOS").Value & "' "
				set xRs2	= ConsultarSQL(query2, Self.Workspace)
				do while not xRs2.EOF
					montoPeriodo = FormatNumber(CSng(xRs2("TOTAL").Value), 2) + montoPeriodo
					xRs2.MoveNext
				loop
				HojaExcel.ActiveSheet.Cells(row, column).Value 		= montoPeriodo
				montoPeriodo = 0
				column = column + 1
				HojaExcel.ActiveSheet.Cells(row, 1).Value 		= xRs("CENTROCOSTOS").Value
				FPerido = DateAdd("m", -countMonth, Date())
			loop
			FPerido = date
			countMonth = 0
			row = row + 1
	...
	HojaExcel.Sheets("ZZ RESUMEN").Name 		= "RESUMEN"
