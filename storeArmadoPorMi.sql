...
BEGIN
	...
	FROM (
		...
		WHERE os.ESTADO <> 'N'
		AND (SELECT TOP 1 ep.id AS CodEP FROM UD_ESTADOFACTURACION AS oef WITH(NOLOCK) 
        LEFT JOIN ITEMTIPOCLASIFICADOR AS ep WITH(NOLOCK) ON oef.ESTADOPAGO_ID = ep.ID
        WHERE oef.BO_PLACE_ID = uos.ESTADOFACTURA_ID 
        ORDER BY oef.FECHA DESC) <> '{6CA42AF2-C52A-4B1F-A7AE-71999839B861}' -- Estado en juicio
